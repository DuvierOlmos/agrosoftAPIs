...
const API_URL = "http://localhost:3001/api/tipoPqrs/admin"; 
...
export const createTipoPqrs = async (tipoPqrs) => {
    try {        
        const response = await axios.post(`${API_URL}/create`, tipoPqrs);        
...
export const updateTipoPqrs = async (id, tipoPqrs) => {
    ...
    try { 
        const response = await axios.put(`${API_URL}/update/${id}`, tipoPqrs);          
